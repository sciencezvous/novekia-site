...
export type SubmissionRateLimitLease = {
  allowed: boolean
  release: () => void
}
...
const MAX_CONCURRENT_PER_FINGERPRINT = 2
const MAX_GLOBAL_REQUESTS_PER_WINDOW = 80
const MAX_GLOBAL_CONCURRENT = 12
...
function cleanup(now: number) {
  ...
  for (const [key, entry] of entries) {
    if (entry.concurrent === 0 && now - entry.lastSeenAt > WINDOW_MS) {
      entries.delete(key)
    }
  }
}

export function acquireConciergeSubmissionRateLimit(
  rawAddress: string,
): SubmissionRateLimitLease {
  const now = Date.now()
  cleanup(now)
